...
#NoPoints Tests
if not (NoPoints.new.eval_prog []).is_a? NoPoints
  puts "NoPoints eval not working properly"
end

if not NoPoints.new.preprocess_prog.is_a? NoPoints
  puts "NoPoints preprocess not working properly"
end

if not NoPoints.new.shift(3,4).is_a? NoPoints
  puts "NoPoints shift not working properly"
end
...
p = Shift.new(ZERO, ZERO, Point.new(ONE, TWO)).eval_prog []
if not (p.x == ONE and p.y == TWO)
  puts "Shift by zero not working"
end

p = Point.new(ONE, ONE)
dx = THREE
dy = FOUR
p1 = Shift.new(dx, dy, p).eval_prog []
if not (p1.x == FOUR and p1.y == FIVE)
  puts "Shifting point not working"
end

vl = VerticalLine.new(THREE)
dx = FOUR
dy = FIVE
vl1 = Shift.new(dx, dy, vl).eval_prog []
if not vl1.x == SEVEN
  puts "Shifting vertical line not working"
end

s = LineSegment.new(ONE, TWO, THREE, FOUR)
dx = TWO
dy = ONE
s = Shift.new(dx, dy, s).eval_prog []
if not (s.x1 == THREE and s.y1 == THREE and s.x2 == FIVE and s.y2 == FIVE)
  puts "Shifting line segment not working"
end

l = Line.new(FOUR, SEVEN)
dx = TWO
dy = THREE
l1 = Shift.new(dx, dy, l).eval_prog []
if not (l1.m == FOUR and l1.b == TWO)
  puts "Shifting line segment not working"
end
