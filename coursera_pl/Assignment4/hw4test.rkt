...
(define tests
  (test-suite
   ...
   (check-equal? (string-append-map (list "Hi" "Mom") "!") '("Hi!" "Mom!") "string-append-map test")
   
   ; list-nth-mod test
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 2) 2 "list-nth-mod test")
   (check-exn exn:fail? (lambda () (list-nth-mod '(0 1 2 3) (- 0 2))))
   (check-exn exn:fail? (lambda () (list-nth-mod null 0)))
   (check-equal? (list-nth-mod '(0 1 2) 5) 2)
   
   ; stream-for-n-steps test
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 1) (list 1) "stream-for-n-steps test")
   (check-equal? (stream-for-n-steps nats 0) null)
   (check-equal? (stream-for-n-steps nats 3) '(1 2 3))
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero nats) 3) (list (cons 0 1) (cons 0 2) (cons 0 3)))
